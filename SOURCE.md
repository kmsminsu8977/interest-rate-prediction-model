...
이 문서는 `interest-rate-prediction-model` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: 금리 예측 모델 (Interest Rate Prediction Model)
- 연구 카테고리: Stochastic Models & Market Risk
- 핵심 질문: 물가, 고용, 기간스프레드, 신용스프레드는 금리 상방/하방 압력을 어떻게 다르게 만드는가?
...
- 시계열 feature table 구성
- 거시 변수 표준화와 방향성 점수화
- 예측 결과를 수준이 아닌 방향성 압력으로 해석하는 방식
